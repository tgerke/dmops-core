...
| DM-P3 — snapshots are immutable and dated | Forbid-mutation triggers on metric_snapshot, source_extract, metric_definition; DML-only app role additionally lacks UPDATE/DELETE on them; every snapshot references its extract checksum | packages/db/migrations/0001_audit_and_views.sql, 0002_app_role.sql |
...
| DM-P5 — role-scoped views over one set of facts | role × study assignment scoping on every read; sponsor serialization excludes internal fields (blocker notes); one underlying view per fact | apps/api/src/routes/, packages/db/migrations/0001 (v_ views) |
| DM-P6 — read-heavy, write-light | Board and summary reads are single-view queries; the only slice-1 write is the milestone PATCH, audited via withActor | apps/api, packages/core/src/milestones.ts |
...
5. **Metric calendar handling is v1.0 calendar-days.** Business-day clocks
   with per-country holiday calendars are a defined, versioned change — not
   silently approximated (ADR-0004).
