...
Environment: commit 324e677, node v23.11.0, 2026-07-30T21:05:34.714Z

Suite result: **PASSED** — 107/107 tests passed.
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE on audit_event at the database level | 18 |
...
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE and DELETE on source_extract | 1 |
...
| PASS | audit trail (ADR-0003) writes an attributed, chained event for every domain mutation | 3 |
| PASS | audit trail (ADR-0003) verifies clean on untampered data | 2 |
| PASS | audit trail (ADR-0003) detects tampering when a row is altered with triggers disabled | 3 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot create tables (no DDL) | 21 |
...
| PASS | DS metric qualification against hand-computed fixtures (stat module, ADR-0012) DS-Q1: pr_review_tat_median v1.0 (business days) matches hand-computed truth for DMOPS-001 | 1 |
...
| PASS | role-scoped views over one set of facts (DM-P5) DM-P5: qa sees the whole portfolio; the sponsor seat sees only its study | 21 |
...
| PASS | UAT cycles and defects (ADR-0010) DM-P6: the analyst's defect write lands and is audit-attributed (ADR-0003) | 11 |
