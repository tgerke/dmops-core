...
Environment: commit 7c6adfc, node v23.11.0, 2026-07-31T14:33:48.530Z

Suite result: **PASSED** — 132/132 tests passed.
...
| PASS | github adapter (recorded fixtures, ADR-0012) maps issues, filtering out pull requests and following Link pagination | 3 |
| PASS | github adapter (recorded fixtures, ADR-0012) derives the three-valued PR state from GitHub's open|closed plus merged_at | 1 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE on audit_event at the database level | 31 |
| PASS | append-only warehouse and audit trail (DM-P3) rejects DELETE on audit_event | 2 |
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE and DELETE on metric_snapshot | 4 |
...
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE and DELETE on milestone_rebaseline (a re-baseline is history, ADR-0009) | 4 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot create tables (no DDL) | 25 |
...
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot write the roster mirrors — display-only, pipeline-written (ADR-0013) | 3 |
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot disable triggers (requires table ownership) | 1 |
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) still audits writes it is allowed to make, attributed via withActor settings | 8 |
...
| PASS | metric qualification against hand-computed fixtures DM-Q9: lock_readiness_pct derives the gate set from the shipped taxonomy and matches hand-computed truth | 22 |
| PASS | metric qualification against hand-computed fixtures DM-Q9: the shipped closure is exactly the eight closeout gates — completing them all scores 100 | 5 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | authentication rejects missing and unknown tokens | 6 |
| PASS | authentication health is public and reports a verified audit chain | 7 |
| PASS | role-scoped views over one set of facts (DM-P5) DM-P5: qa sees the whole portfolio; the sponsor seat sees only its study | 38 |
...
| PASS | role-scoped views over one set of facts (DM-P5) DM-P6: the board is one read; planned/forecast/actual arrive as the triple, never collapsed | 4 |
| PASS | milestone writes (ADR-0003, ADR-0008) read-only roles cannot write: clinops and sponsor get 403 | 4 |
| PASS | milestone writes (ADR-0003, ADR-0008) the DM lead's write lands, returns the board row, and is audit-attributed to them | 25 |
| PASS | milestone writes (ADR-0003, ADR-0008) ADR-0008: baseline_date and planned_date are not writable through the API | 1 |
| PASS | milestone writes (ADR-0003, ADR-0008) 404s on a milestone occurrence the study does not have | 3 |
...
| PASS | deliverable surface (ADR-0006) DM-P6: read-only roles cannot write deliverable status; the DM lead's write is audit-attributed (ADR-0003) | 19 |
...
| PASS | re-baselining governance (ADR-0009, ADR-0003) DM-P6: the dm_manager's re-baseline moves planned_date, never baseline_date, and both writes are audit-attributed (ADR-0003) | 12 |
| PASS | re-baselining governance (ADR-0009, ADR-0003) a complete milestone cannot be re-baselined; nor can one with a throwaway reason | 3 |
...
| PASS | UAT cycles and defects (ADR-0010) DM-P4: UAT serves cycle status, counts, and an evidence pointer — never scripts, screenshots, or signatures | 4 |
| PASS | UAT cycles and defects (ADR-0010) DM-P5: defect reads are row-scoped; the sponsor serialization omits resolution notes | 4 |
| PASS | UAT cycles and defects (ADR-0010) DM-P6: the analyst's defect write lands and is audit-attributed (ADR-0003) | 13 |
| PASS | UAT cycles and defects (ADR-0010) read-only roles cannot write UAT: clinops and sponsor get 403; the analyst is study-scoped | 10 |
| PASS | UAT cycles and defects (ADR-0010) UAT.COMPLETE means defects resolved: completing a cycle with open defects is rejected (ADR-0010) | 47 |
| PASS | UAT cycles and defects (ADR-0010) endings are dated facts: resolved without a date and closed without a substantive note are rejected | 8 |
| PASS | UAT cycles and defects (ADR-0010) ADR-0010: identity fields are not writable, and a finished cycle takes no new defects | 10 |
...
| PASS | stat module (ADR-0011) DM-P6: the biostatistician writes analysis milestones, but DM-phase milestones stay leadership-only | 12 |
...
| PASS | stat module (ADR-0011) analysis deliverable types accept the analysis posture; DM types do not (ADR-0011) | 13 |
| PASS | stat module (ADR-0011) DM-P1: a stat-module study serves the dm set plus the DS starter set (ADR-0012) | 30 |
| PASS | stat module (ADR-0011) DM-P1: the module boundary hides stat metrics from a dm-only study entirely (ADR-0011) | 11 |
| PASS | training and access mirrors (ADR-0013) DM-P1: the roster is mirrored from the source, one row per person, grants aggregated | 5 |
...
| PASS | training and access mirrors (ADR-0013) DM-P4: training records serve dated status and provenance, never certificates | 11 |
| PASS | training and access mirrors (ADR-0013) DM-P5: mirror reads are row-scoped; the sponsor sees the roster of its own study | 7 |
| PASS | training and access mirrors (ADR-0013) a study with no roster-capable source serves empty mirrors, not errors | 8 |
| PASS | training and access mirrors (ADR-0013) DM-Q7/DM-Q8: the roster metrics flow through the snapshot pipeline with the fixture truth | 17 |
| PASS | lock-readiness (ADR-0014) DM-P1: the checklist is the depends_on closure of CLOSE.LOCK, derived — never entered | 11 |
...
| PASS | lock-readiness (ADR-0014) DM-Q9: lock_readiness_pct flows through the snapshot pipeline as a dmops-native metric | 14 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P2: every dictionary metric appears with its version and availability | 7 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P1: a study without a source reports adapter metrics unavailable, not zero | 8 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P3: snapshot history is served from immutable rows with extract lineage | 4 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P3: study-grain history spans reporting periods, newest first | 3 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P2: the site drill-down serves the same versioned metric at site grain | 7 |
