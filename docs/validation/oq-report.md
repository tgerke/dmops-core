...
Environment: commit be47681, node v23.11.0, 2026-07-31T18:46:17.160Z

Suite result: **PASSED** — 172/172 tests passed.
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE on audit_event at the database level | 26 |
...
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) still audits writes it is allowed to make, attributed via withActor settings | 6 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | portfolio pooling declarations (ADR-0015) DM-P2: the pooling enumeration covers exactly the governed dictionary — a new metric must declare its portfolio behavior | 15 |
...
| PASS | metric qualification against hand-computed fixtures DM-Q5: query_tat_median v1.2 (holiday-aware) matches hand-computed truth under the shipped calendar | 1 |
...
| PASS | milestone writes (ADR-0003, ADR-0008) read-only roles cannot write: clinops and sponsor get 403 | 7 |
| PASS | milestone writes (ADR-0003, ADR-0008) the DM lead's write lands, returns the board row, and is audit-attributed to them | 19 |
...
| PASS | re-baselining governance (ADR-0009, ADR-0003) DM-P6: the dm_manager's re-baseline moves planned_date, never baseline_date, and both writes are audit-attributed (ADR-0003) | 17 |
...
| PASS | UAT cycles and defects (ADR-0010) read-only roles cannot write UAT: clinops and sponsor get 403; the analyst is study-scoped | 3 |
| PASS | UAT cycles and defects (ADR-0010) UAT.COMPLETE means defects resolved: completing a cycle with open defects is rejected (ADR-0010) | 15 |
...
| PASS | stat module (ADR-0011) DM-P6: the programmer's analysis-phase write lands and is audit-attributed (ADR-0003) | 6 |
| PASS | stat module (ADR-0011) DM-P6: the biostatistician writes analysis milestones, but DM-phase milestones stay leadership-only | 5 |
...
| PASS | stat module (ADR-0011) analysis deliverable types accept the analysis posture; DM types do not (ADR-0011) | 6 |
| PASS | stat module (ADR-0011) DM-P1: a stat-module study serves the dm set plus the DS starter set (ADR-0012) | 17 |
| PASS | stat module (ADR-0011) DM-P1: the module boundary hides stat metrics from a dm-only study entirely (ADR-0011) | 6 |
...
| PASS | lock-readiness (ADR-0014) signals ride beside the score and never move it: the score is 0 while the evidence shows live work | 5 |
| PASS | lock-readiness (ADR-0014) DM-P5: the sponsor serialization omits gate blocker notes, and reads are row-scoped | 4 |
| PASS | lock-readiness (ADR-0014) a study with no wired sources serves named absence for signals, not fake zeros (ADR-0005) | 2 |
| PASS | lock-readiness (ADR-0014) DM-Q9: lock_readiness_pct flows through the snapshot pipeline as a dmops-native metric | 5 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P2: every dictionary metric appears with its version and availability | 5 |
...
| PASS | portfolio roll-up (ADR-0015) DM-P5: the portfolio is one fact at portfolio grain — portfolio readers see it, study-scoped seats get 403, not a smaller number | 2517 |
| PASS | portfolio roll-up (ADR-0015) DM-P2: every dictionary metric appears once with a declared pooling kind, scoped to the studies that enabled its module | 1146 |
| PASS | portfolio roll-up (ADR-0015) DM-P3: ratio metrics pool exactly from stored numerators and denominators — 0 of 16 lock gates across the portfolio | 2327 |
| PASS | portfolio roll-up (ADR-0015) medians never pool — a named absence with the per-study spread, not a fake portfolio median (ADR-0005) | 2481 |
| PASS | portfolio roll-up (ADR-0015) DM-P1: a metric no source can feed reports its honest scope — one of two studies reporting, pooled over the reporting study only | 1162 |
| PASS | portfolio roll-up (ADR-0015) DM-Q9: the readiness burn-up serves one pooled point per reporting period from the monthly snapshots | 1185 |
| PASS | exports and KPI packs (ADR-0016) DM-P3: the snapshot CSV is the immutable history flattened, provenance columns included | 4 |
| PASS | exports and KPI packs (ADR-0016) DM-P5: the snapshot CSV is row-scoped exactly like the JSON it flattens | 2 |
| PASS | exports and KPI packs (ADR-0016) DM-P5: the portfolio CSV requires portfolio read and keeps the named absences | 1155 |
...
| PASS | exports and KPI packs (ADR-0016) DM-P2: the pack is period-scoped — May on request, 404 for a period never computed | 17 |
| PASS | exports and KPI packs (ADR-0016) DM-P1/DM-P5: a sourceless study's pack names its absences; the pack is row-scoped | 11 |
