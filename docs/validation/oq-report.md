...
Environment: commit 7600a13, node v23.11.0, 2026-08-07T15:58:47.752Z

Suite result: **PASSED** — 215/215 tests passed.
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | portfolio pooling declarations (ADR-0015) DM-P2: the pooling enumeration covers exactly the governed dictionary — a new metric must declare its portfolio behavior | 26 |
| PASS | portfolio pooling declarations (ADR-0015) an undeclared metric is a hard error, never a silently unpooled card | 1 |

## packages/db/src/immutability.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE on audit_event at the database level | 30 |
| PASS | append-only warehouse and audit trail (DM-P3) rejects DELETE on audit_event | 2 |
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE and DELETE on metric_snapshot | 3 |
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE and DELETE on source_extract | 3 |
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE and DELETE on metric_definition (a changed definition is a new version) | 3 |
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE and DELETE on milestone_rebaseline (a re-baseline is history, ADR-0009) | 3 |
...
| PASS | audit trail (ADR-0003) detects tampering when a row is altered with triggers disabled | 9 |
| PASS | display-only posture (DM-P4) has no signature columns anywhere in the schema | 6 |
| PASS | display-only posture (DM-P4) stores deliverable evidence as an eTMF pointer, not content | 3 |

## packages/db/src/privileges.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot create tables (no DDL) | 23 |
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot INSERT audit_event directly (cannot fabricate audit) | 3 |
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot TRUNCATE domain tables | 1 |
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot UPDATE or DELETE metric_snapshot even before the trigger fires (DM-P3) | 4 |
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot UPDATE or DELETE milestone_rebaseline even before the trigger fires (ADR-0009) | 4 |
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot write the roster mirrors — display-only, pipeline-written (ADR-0013) | 7 |
...
| PASS | metric qualification against hand-computed fixtures DM-Q9: lock_readiness_pct derives the gate set from the shipped taxonomy and matches hand-computed truth | 9 |
| PASS | metric qualification against hand-computed fixtures DM-Q9: the shipped closure is exactly the eight closeout gates — completing them all scores 100 | 6 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | csv adapter (the reference implementation) extracts all four frames from the fixture study and passes contract validation | 9 |
...
| PASS | rave adapter (recorded fixtures, ADR-0017) reconstructs query lifecycles from the audit tape, following the Link cursor | 6 |
...
| PASS | rave adapter (recorded fixtures, ADR-0017) maps the subjects listing through statusMap and SiteRef (ADR-0017) | 1 |
...
| PASS | rave adapter (recorded fixtures, ADR-0017) fails loudly when a value does not parse under the declared dateFormat (ADR-0017/ADR-0018) | 1 |
| PASS | rave adapter (recorded fixtures, ADR-0017) signs every request — Link pages included — with verifiable V1+V2 headers (ADR-0021) | 10 |
| PASS | rave adapter (recorded fixtures, ADR-0017) capability posture is independent of auth mode (ADR-0021) | 0 |
| PASS | rave adapter (recorded fixtures, ADR-0017) refuses a config carrying both auth modes, or neither (ADR-0021) | 0 |
| PASS | rave adapter (recorded fixtures, ADR-0017) fails with an actionable message when the private-key env var is missing | 0 |
...
## packages/adapters/src/rave/mauth.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWS/binary-body | 3 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWS/get-unreserved | 2 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWS/get-vanilla | 2 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWS/post-vanilla-body | 2 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWS/post-vanilla-body-empty | 2 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/authentication-only-after-decoding | 2 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/binary-body | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-normalize-multiple-slashes | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-normalize-parent-in-path | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-normalize-self-in-path | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-unreserved | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-utf8-normalize-case | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-vanilla | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-vanilla-query-encoded-tilde | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-vanilla-query-no-value | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-vanilla-query-param-key-ordering | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-vanilla-query-param-value-ordering | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-vanilla-query-space | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-vanilla-query-unreserved | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-vanilla-query-utf8 | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/get-vanilla-query-utf8-normalize-case | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/post-vanilla-body | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/post-vanilla-body-empty | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) MWSV2/post-vanilla-body-query | 1 |
| PASS | mauth signer conformance (vendored mdsol/mauth-protocol-test-suite, ADR-0021) emits all four headers, both protocols, one call (vendor client default posture) | 2 |

## apps/api/src/app.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | authentication rejects missing and unknown tokens | 8 |
| PASS | authentication health is public and reports a verified audit chain | 8 |
| PASS | role-scoped views over one set of facts (DM-P5) DM-P5: qa sees the whole portfolio; the sponsor seat sees only its study | 28 |
| PASS | role-scoped views over one set of facts (DM-P5) DM-P5: the sponsor serialization carries no blocker notes; the DM lead's does | 11 |
| PASS | role-scoped views over one set of facts (DM-P5) DM-P6: the board is one read; planned/forecast/actual arrive as the triple, never collapsed | 5 |
| PASS | milestone writes (ADR-0003, ADR-0008) read-only roles cannot write: clinops and sponsor get 403 | 6 |
| PASS | milestone writes (ADR-0003, ADR-0008) the DM lead's write lands, returns the board row, and is audit-attributed to them | 42 |
| PASS | milestone writes (ADR-0003, ADR-0008) ADR-0008: baseline_date and planned_date are not writable through the API | 2 |
| PASS | milestone writes (ADR-0003, ADR-0008) 404s on a milestone occurrence the study does not have | 4 |
| PASS | deliverable surface (ADR-0006) DM-P4: deliverables serve status and an eTMF pointer, never content or signatures | 3 |
| PASS | deliverable surface (ADR-0006) DM-P5: deliverable reads are row-scoped; the sponsor seat sees its study only | 3 |
| PASS | deliverable surface (ADR-0006) DM-P6: read-only roles cannot write deliverable status; the DM lead's write is audit-attributed (ADR-0003) | 15 |
| PASS | deliverable surface (ADR-0006) approving without an approved_date is rejected: approvals are dated facts (ADR-0006) | 6 |
| PASS | deliverable surface (ADR-0006) ADR-0006: identity fields are not writable and unknown fields are rejected | 3 |
| PASS | re-baselining governance (ADR-0009, ADR-0003) re-baselining is above routine edits: dm_lead, clinops, and sponsor get 403 | 4 |
| PASS | re-baselining governance (ADR-0009, ADR-0003) DM-P6: the dm_manager's re-baseline moves planned_date, never baseline_date, and both writes are audit-attributed (ADR-0003) | 24 |
| PASS | re-baselining governance (ADR-0009, ADR-0003) a complete milestone cannot be re-baselined; nor can one with a throwaway reason | 6 |
| PASS | re-baselining governance (ADR-0009, ADR-0003) DM-P5: re-baseline history serves dates to everyone; reasons are omitted from the sponsor serialization | 4 |
| PASS | re-baselining governance (ADR-0009, ADR-0003) 404s on a milestone the study does not have | 2 |
| PASS | UAT cycles and defects (ADR-0010) DM-P4: UAT serves cycle status, counts, and an evidence pointer — never scripts, screenshots, or signatures | 5 |
| PASS | UAT cycles and defects (ADR-0010) DM-P5: defect reads are row-scoped; the sponsor serialization omits resolution notes | 6 |
| PASS | UAT cycles and defects (ADR-0010) DM-P6: the analyst's defect write lands and is audit-attributed (ADR-0003) | 17 |
| PASS | UAT cycles and defects (ADR-0010) read-only roles cannot write UAT: clinops and sponsor get 403; the analyst is study-scoped | 7 |
| PASS | UAT cycles and defects (ADR-0010) UAT.COMPLETE means defects resolved: completing a cycle with open defects is rejected (ADR-0010) | 35 |
| PASS | UAT cycles and defects (ADR-0010) endings are dated facts: resolved without a date and closed without a substantive note are rejected | 6 |
...
| PASS | stat module (ADR-0011) DM-P5: the board serves analysis rows only where the module is enabled | 5 |
| PASS | stat module (ADR-0011) DM-P6: the programmer's analysis-phase write lands and is audit-attributed (ADR-0003) | 8 |
| PASS | stat module (ADR-0011) DM-P6: the biostatistician writes analysis milestones, but DM-phase milestones stay leadership-only | 9 |
...
| PASS | stat module (ADR-0011) analysis deliverable types accept the analysis posture; DM types do not (ADR-0011) | 9 |
| PASS | stat module (ADR-0011) DM-P1: a stat-module study serves the dm set plus the DS starter set (ADR-0012) | 26 |
...
| PASS | training and access mirrors (ADR-0013) DM-P5: mirror reads are row-scoped; the sponsor sees the roster of its own study | 3 |
| PASS | training and access mirrors (ADR-0013) a study with no roster-capable source serves empty mirrors, not errors | 3 |
| PASS | training and access mirrors (ADR-0013) DM-Q7/DM-Q8: the roster metrics flow through the snapshot pipeline with the fixture truth | 7 |
| PASS | lock-readiness (ADR-0014) DM-P1: the checklist is the depends_on closure of CLOSE.LOCK, derived — never entered | 7 |
| PASS | lock-readiness (ADR-0014) signals ride beside the score and never move it: the score is 0 while the evidence shows live work | 7 |
| PASS | lock-readiness (ADR-0014) DM-P5: the sponsor serialization omits gate blocker notes, and reads are row-scoped | 14 |
| PASS | lock-readiness (ADR-0014) a study with no wired sources serves named absence for signals, not fake zeros (ADR-0005) | 8 |
| PASS | lock-readiness (ADR-0014) DM-Q9: lock_readiness_pct flows through the snapshot pipeline as a dmops-native metric | 20 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P2: every dictionary metric appears with its version and availability | 22 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P1: a study without a source reports adapter metrics unavailable, not zero | 10 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P1: an EDC-only study names the training frame's missing feeder for the mirror-fed metric (ADR-0019) | 11 |
...
| PASS | portfolio roll-up (ADR-0015) DM-P5: the portfolio is one fact at portfolio grain — portfolio readers see it, study-scoped seats get 403, not a smaller number | 37 |
| PASS | portfolio roll-up (ADR-0015) DM-P2: every dictionary metric appears once with a declared pooling kind, scoped to the studies that enabled its module | 10 |
| PASS | portfolio roll-up (ADR-0015) DM-P3: ratio metrics pool exactly from stored numerators and denominators — 0 of 16 lock gates across the portfolio | 20 |
| PASS | portfolio roll-up (ADR-0015) medians never pool — a named absence with the per-study spread, not a fake portfolio median (ADR-0005) | 20 |
| PASS | portfolio roll-up (ADR-0015) DM-P1: a metric no source can feed reports its honest scope — one of two studies reporting, pooled over the reporting study only | 8 |
| PASS | portfolio roll-up (ADR-0015) DM-Q9: the readiness burn-up serves one pooled point per reporting period from the monthly snapshots | 13 |
...
| PASS | exports and KPI packs (ADR-0016) DM-P5: the snapshot CSV is row-scoped exactly like the JSON it flattens | 2 |
| PASS | exports and KPI packs (ADR-0016) DM-P5: the portfolio CSV requires portfolio read and keeps the named absences | 10 |
| PASS | exports and KPI packs (ADR-0016) DM-P2: the pack serves each metric's registered definition at the computed version, with extract citations | 17 |
| PASS | exports and KPI packs (ADR-0016) DM-P2: the pack is period-scoped — May on request, 404 for a period never computed | 15 |
| PASS | exports and KPI packs (ADR-0016) DM-P1/DM-P5: a sourceless study's pack names its absences; the pack is row-scoped | 9 |
