...
Environment: commit f60f309, node v23.11.0, 2026-07-31T17:58:36.672Z

Suite result: **PASSED** — 167/167 tests passed.
...
| PASS | extraction validation names the frame, row, and field on failure | 1 |
| PASS | checksum determinism (extract provenance, ADR-0007) is stable across key order | 1 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE on audit_event at the database level | 35 |
...
| PASS | append-only warehouse and audit trail (DM-P3) rejects UPDATE and DELETE on milestone_rebaseline (a re-baseline is history, ADR-0009) | 5 |
| PASS | audit trail (ADR-0003) writes an attributed, chained event for every domain mutation | 10 |
| PASS | audit trail (ADR-0003) verifies clean on untampered data | 4 |
| PASS | audit trail (ADR-0003) detects tampering when a row is altered with triggers disabled | 8 |
| PASS | display-only posture (DM-P4) has no signature columns anywhere in the schema | 7 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot create tables (no DDL) | 34 |
...
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) cannot UPDATE or DELETE milestone_rebaseline even before the trigger fires (ADR-0009) | 9 |
...
| PASS | runtime role privilege ceilings (ADR-0003, DM-P3) still audits writes it is allowed to make, attributed via withActor settings | 10 |

## packages/metrics/src/engine.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | metric dictionary (DM-P2) every YAML definition has a registered compute function of the same version, and vice versa | 2 |
...
| PASS | capability gating (DM-P1: skip, never silently approximate) the medrio adapter's honest capabilities gate off the whole EDC metric set with named gaps (ADR-0017, DM-P1) | 1 |
| PASS | capability gating (DM-P1: skip, never silently approximate) the rave adapter lights the query metrics as derived and keeps entry_lag off (ADR-0017, DM-P1) | 1 |

## packages/metrics/src/pooling.test.ts

| Result | Test | ms |
| --- | --- | ---: |
| PASS | portfolio pooling declarations (ADR-0015) DM-P2: the pooling enumeration covers exactly the governed dictionary — a new metric must declare its portfolio behavior | 21 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | csv adapter (the reference implementation) extracts all four frames from the fixture study and passes contract validation | 14 |
...
| PASS | rave adapter (recorded fixtures, ADR-0017) reconstructs query lifecycles from the audit tape, following the Link cursor | 7 |
...
| PASS | rave adapter (recorded fixtures, ADR-0017) fails with an actionable message when a credential env var is missing | 3 |
...
| PASS | metric qualification against hand-computed fixtures DM-Q4: milestone_slip matches hand-computed truth on constructed milestone facts | 1 |
...
| PASS | metric qualification against hand-computed fixtures DM-Q9: lock_readiness_pct derives the gate set from the shipped taxonomy and matches hand-computed truth | 10 |
| PASS | metric qualification against hand-computed fixtures DM-Q9: the shipped closure is exactly the eight closeout gates — completing them all scores 100 | 7 |
...
| Result | Test | ms |
| --- | --- | ---: |
| PASS | authentication rejects missing and unknown tokens | 7 |
| PASS | authentication health is public and reports a verified audit chain | 9 |
| PASS | role-scoped views over one set of facts (DM-P5) DM-P5: qa sees the whole portfolio; the sponsor seat sees only its study | 44 |
| PASS | role-scoped views over one set of facts (DM-P5) DM-P5: the sponsor serialization carries no blocker notes; the DM lead's does | 97 |
| PASS | role-scoped views over one set of facts (DM-P5) DM-P6: the board is one read; planned/forecast/actual arrive as the triple, never collapsed | 12 |
| PASS | milestone writes (ADR-0003, ADR-0008) read-only roles cannot write: clinops and sponsor get 403 | 13 |
| PASS | milestone writes (ADR-0003, ADR-0008) the DM lead's write lands, returns the board row, and is audit-attributed to them | 56 |
| PASS | milestone writes (ADR-0003, ADR-0008) ADR-0008: baseline_date and planned_date are not writable through the API | 2 |
| PASS | milestone writes (ADR-0003, ADR-0008) 404s on a milestone occurrence the study does not have | 6 |
| PASS | deliverable surface (ADR-0006) DM-P4: deliverables serve status and an eTMF pointer, never content or signatures | 7 |
| PASS | deliverable surface (ADR-0006) DM-P5: deliverable reads are row-scoped; the sponsor seat sees its study only | 7 |
| PASS | deliverable surface (ADR-0006) DM-P6: read-only roles cannot write deliverable status; the DM lead's write is audit-attributed (ADR-0003) | 28 |
| PASS | deliverable surface (ADR-0006) approving without an approved_date is rejected: approvals are dated facts (ADR-0006) | 15 |
| PASS | deliverable surface (ADR-0006) ADR-0006: identity fields are not writable and unknown fields are rejected | 11 |
| PASS | re-baselining governance (ADR-0009, ADR-0003) re-baselining is above routine edits: dm_lead, clinops, and sponsor get 403 | 6 |
| PASS | re-baselining governance (ADR-0009, ADR-0003) DM-P6: the dm_manager's re-baseline moves planned_date, never baseline_date, and both writes are audit-attributed (ADR-0003) | 20 |
| PASS | re-baselining governance (ADR-0009, ADR-0003) a complete milestone cannot be re-baselined; nor can one with a throwaway reason | 4 |
| PASS | re-baselining governance (ADR-0009, ADR-0003) DM-P5: re-baseline history serves dates to everyone; reasons are omitted from the sponsor serialization | 8 |
| PASS | re-baselining governance (ADR-0009, ADR-0003) 404s on a milestone the study does not have | 4 |
| PASS | UAT cycles and defects (ADR-0010) DM-P4: UAT serves cycle status, counts, and an evidence pointer — never scripts, screenshots, or signatures | 7 |
| PASS | UAT cycles and defects (ADR-0010) DM-P5: defect reads are row-scoped; the sponsor serialization omits resolution notes | 7 |
| PASS | UAT cycles and defects (ADR-0010) DM-P6: the analyst's defect write lands and is audit-attributed (ADR-0003) | 17 |
| PASS | UAT cycles and defects (ADR-0010) read-only roles cannot write UAT: clinops and sponsor get 403; the analyst is study-scoped | 5 |
| PASS | UAT cycles and defects (ADR-0010) UAT.COMPLETE means defects resolved: completing a cycle with open defects is rejected (ADR-0010) | 30 |
| PASS | UAT cycles and defects (ADR-0010) endings are dated facts: resolved without a date and closed without a substantive note are rejected | 10 |
| PASS | UAT cycles and defects (ADR-0010) ADR-0010: identity fields are not writable, and a finished cycle takes no new defects | 6 |
| PASS | stat module (ADR-0011) DM-P5: the board serves analysis rows only where the module is enabled | 10 |
| PASS | stat module (ADR-0011) DM-P6: the programmer's analysis-phase write lands and is audit-attributed (ADR-0003) | 15 |
| PASS | stat module (ADR-0011) DM-P6: the biostatistician writes analysis milestones, but DM-phase milestones stay leadership-only | 11 |
...
| PASS | stat module (ADR-0011) analysis deliverable types accept the analysis posture; DM types do not (ADR-0011) | 15 |
| PASS | stat module (ADR-0011) DM-P1: a stat-module study serves the dm set plus the DS starter set (ADR-0012) | 46 |
| PASS | stat module (ADR-0011) DM-P1: the module boundary hides stat metrics from a dm-only study entirely (ADR-0011) | 19 |
| PASS | training and access mirrors (ADR-0013) DM-P1: the roster is mirrored from the source, one row per person, grants aggregated | 4 |
| PASS | training and access mirrors (ADR-0013) training_gap flags the inspection question: expired, overdue, and missing training on active access | 3 |
| PASS | training and access mirrors (ADR-0013) DM-P4: training records serve dated status and provenance, never certificates | 4 |
| PASS | training and access mirrors (ADR-0013) DM-P5: mirror reads are row-scoped; the sponsor sees the roster of its own study | 5 |
| PASS | training and access mirrors (ADR-0013) a study with no roster-capable source serves empty mirrors, not errors | 4 |
| PASS | training and access mirrors (ADR-0013) DM-Q7/DM-Q8: the roster metrics flow through the snapshot pipeline with the fixture truth | 9 |
| PASS | lock-readiness (ADR-0014) DM-P1: the checklist is the depends_on closure of CLOSE.LOCK, derived — never entered | 7 |
| PASS | lock-readiness (ADR-0014) signals ride beside the score and never move it: the score is 0 while the evidence shows live work | 4 |
| PASS | lock-readiness (ADR-0014) DM-P5: the sponsor serialization omits gate blocker notes, and reads are row-scoped | 5 |
| PASS | lock-readiness (ADR-0014) a study with no wired sources serves named absence for signals, not fake zeros (ADR-0005) | 4 |
| PASS | lock-readiness (ADR-0014) DM-Q9: lock_readiness_pct flows through the snapshot pipeline as a dmops-native metric | 11 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P2: every dictionary metric appears with its version and availability | 9 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P1: a study without a source reports adapter metrics unavailable, not zero | 11 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P3: snapshot history is served from immutable rows with extract lineage | 3 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P3: study-grain history spans reporting periods, newest first | 2 |
| PASS | metrics surface (DM-P1, DM-P2, DM-P3) DM-P2: the site drill-down serves the same versioned metric at site grain | 3 |
...
| PASS | portfolio roll-up (ADR-0015) DM-P5: the portfolio is one fact at portfolio grain — portfolio readers see it, study-scoped seats get 403, not a smaller number | 4133 |
| PASS | portfolio roll-up (ADR-0015) DM-P2: every dictionary metric appears once with a declared pooling kind, scoped to the studies that enabled its module | 1702 |
| PASS | portfolio roll-up (ADR-0015) DM-P3: ratio metrics pool exactly from stored numerators and denominators — 0 of 16 lock gates across the portfolio | 3532 |
| PASS | portfolio roll-up (ADR-0015) medians never pool — a named absence with the per-study spread, not a fake portfolio median (ADR-0005) | 4006 |
| PASS | portfolio roll-up (ADR-0015) DM-P1: a metric no source can feed reports its honest scope — one of two studies reporting, pooled over the reporting study only | 2008 |
| PASS | portfolio roll-up (ADR-0015) DM-Q9: the readiness burn-up serves one pooled point per reporting period from the monthly snapshots | 1888 |
| PASS | exports and KPI packs (ADR-0016) DM-P3: the snapshot CSV is the immutable history flattened, provenance columns included | 8 |
| PASS | exports and KPI packs (ADR-0016) DM-P5: the snapshot CSV is row-scoped exactly like the JSON it flattens | 4 |
| PASS | exports and KPI packs (ADR-0016) DM-P5: the portfolio CSV requires portfolio read and keeps the named absences | 2420 |
| PASS | exports and KPI packs (ADR-0016) DM-P2: the pack serves each metric's registered definition at the computed version, with extract citations | 38 |
| PASS | exports and KPI packs (ADR-0016) DM-P2: the pack is period-scoped — May on request, 404 for a period never computed | 39 |
| PASS | exports and KPI packs (ADR-0016) DM-P1/DM-P5: a sourceless study's pack names its absences; the pack is row-scoped | 29 |
