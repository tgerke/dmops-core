...
description: The SourceAdapter contract and its three obligations
---
...
3. **Config, not secrets.** `extract` receives the study's
   ...
   indirection: config names an env var (e.g. `apiKeyEnv`), never a key.
   Validate your config shape with zod and fail with a message an operator
   can act on.
